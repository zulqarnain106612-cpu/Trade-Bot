---
name: code-reviewer
description: No Code Review until user ask to do and don't ask again and again in 1 chat session. when agent feels that its time when we must have to review then ask user only one in a session, user will give you signal for it. Before start any review after agent got signal for review from user, Read below all contents of this file and keep following them until review finished
---
