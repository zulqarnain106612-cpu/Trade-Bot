...
import { convert, toArrayBuffer, assertBufferSource, toUint8Array } from '@peculiar/utils';
import { AsnParser, AsnSerializer, AsnConvert } from '@peculiar/asn1-schema';

const JsonBase64UrlConverter = {
    fromJSON: (value) => Buffer$1.from(convert.decode("base64url", value)),
    toJSON: (value) => convert.encode("base64url", value),
};
...
class AesCrypto {
    static AES_KW_IV = Buffer$1.from("A6A6A6A6A6A6A6A6", "hex");
    ...
    static async importKey(format, keyData, algorithm, extractable, keyUsages) {
        ...
        switch (format.toLowerCase()) {
            ...
            case "raw":
                ...
                key.data = Buffer$1.from(toUint8Array(keyData));
                break;
            default:
                throw new core.OperationError("format: Must be 'jwk' or 'raw'");
        }
    ...
    static async encryptAesCTR(algorithm, key, data) {
        const cipher = crypto.createCipheriv(`aes-${key.algorithm.length}-ctr`, key.data, Buffer$1.from(toUint8Array(algorithm.counter)));
        let enc = cipher.update(data);
        enc = Buffer$1.concat([enc, cipher.final()]);
        const res = toArrayBuffer(enc);
        return res;
    ...
    static async encryptAesGCM(algorithm, key, data) {
        const cipher = crypto.createCipheriv(`aes-${key.algorithm.length}-gcm`, key.data, Buffer$1.from(toUint8Array(algorithm.iv)), { authTagLength: (algorithm.tagLength || 128) >> 3 });
        if (algorithm.additionalData) {
            cipher.setAAD(Buffer$1.from(toUint8Array(algorithm.additionalData)));
        }
        let enc = cipher.update(data);
    ...
    static async decryptAesGCM(algorithm, key, data) {
        ...
        const decipher = crypto.createDecipheriv(`aes-${key.algorithm.length}-gcm`, key.data, toUint8Array(algorithm.iv), { authTagLength: tagLength });
        const enc = data.slice(0, data.length - tagLength);
        const tag = data.slice(data.length - tagLength);
        if (algorithm.additionalData) {
            decipher.setAAD(Buffer$1.from(toUint8Array(algorithm.additionalData)));
        }
...
class DesCrypto {
    ...
    static async encryptDesEDE3CBC(algorithm, key, data) {
        const cipher = crypto.createCipheriv("des-ede3-cbc", key.data, Buffer$1.from(toUint8Array(algorithm.iv)));
        let enc = cipher.update(data);
        enc = Buffer$1.concat([enc, cipher.final()]);
        const res = toArrayBuffer(enc);
        return res;
...
class RsaOaepProvider extends core.RsaOaepProvider {
    ...
    async onDecrypt(algorithm, key, data) {
        ...
        let pkcs0 = crypto.privateDecrypt({
            key: internalKey.pem,
            padding: crypto.constants.RSA_NO_PADDING,
        }, Buffer$1.from(toUint8Array(data)));
        const z = pkcs0[0];
...
class EcdsaProvider extends core.EcdsaProvider {
    namedCurves = core.EcCurves.names;
    hashAlgorithms = [
        "SHA-1", "SHA-256", "SHA-384", "SHA-512",
        "shake128", "shake256",
        "SHA3-256", "SHA3-384", "SHA3-512"
...
class HmacProvider extends core.HmacProvider {
    ...
    async onSign(algorithm, key, data) {
        ...
        const hmac = crypto.createHmac(cryptoAlg, getCryptoKey(key).data)
            .update(Buffer$1.from(toUint8Array(data))).digest();
    ...
    async onVerify(algorithm, key, signature, data) {
        ...
        const hmac = crypto.createHmac(cryptoAlg, getCryptoKey(key).data)
            .update(Buffer$1.from(toUint8Array(data))).digest();
        return hmac.compare(Buffer$1.from(toUint8Array(signature))) === 0;
    }
    async onImportKey(format, keyData, algorithm, extractable, keyUsages) {
        let key;
        switch (format.toLowerCase()) {
...
class HkdfProvider extends core.HkdfProvider {
    ...
    async onDeriveBits(params, baseKey, length) {
        ...
        return BufferSourceConverter.toArrayBuffer(Buffer$1.concat(blocks).slice(0, byteLength));
    }
    checkCryptoKey(key, keyUsage) {
        super.checkCryptoKey(key, keyUsage);
