...
## Debt-012 [2026-07-09] — NEW (static audit, Claude)
`requirements.lock` was generated by `pip-compile` from `requirements.in`, but `requirements.in`
was missing `aiohttp` and `prometheus_client` — both of which are direct runtime dependencies
imported in `src/`. This means the lock file produced by `pip-compile --generate-hashes` would
not include those packages at the top-level hash-pinned level, so a CI install with
`--require-hashes -r requirements.lock` would either fail or pull them in as transitive
(without the determinism guarantee). Also `watchdog` appeared in `requirements.txt` and
`requirements-dev.txt` but is not imported anywhere in `src/` — dead dependency adding
unnecessary install surface.
Fix applied [2026-07-09]: `aiohttp>=3.9,<4.0` and `prometheus_client>=0.20,<1.0` added to
`requirements.in` and `requirements.txt`. `watchdog` removed from both files. Lock file
should be regenerated on host: `pip-compile --generate-hashes requirements.in -o requirements.lock`
Severity: Medium (CI install reproducibility gap — installs work locally but lock integrity is undermined)
File: `requirements.txt`, `requirements.in`, `requirements-dev.txt`, `requirements.lock`
Status: PARTIALLY RESOLVED — source files corrected; lock regeneration requires host pip-compile run.
