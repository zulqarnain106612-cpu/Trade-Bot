...
class QuantValidator:
    ...
    def validate(self, ctx: SignalContext) -> ValidatorResult:
        ...
        # 1a. Kelly fraction — continuous return form: f* = μ/σ²
        #     μ = expected_edge_bps / 10000 (per-trade)
        #     σ = realized_vol / sqrt(252 * bars_per_day)
        mu    = ctx.expected_edge_bps / 10_000.0
        sigma = ctx.realized_vol / math.sqrt(252)
        if sigma <= 0:
            return ValidatorResult(
                self.NAME, ValidatorStatus.VETO,
        ...
        # Reduce fraction if realized_vol > 2× historical baseline (vol explosion)
        ...
        metrics.update({"kelly_final": round(kelly_capped, 6), "p_bet": ctx.p_bet,
                         "p_long": ctx.p_long, "expected_edge_bps": ctx.expected_edge_bps})
        return ValidatorResult(
            self.NAME, ValidatorStatus.PASS,
...
class ProbabilityValidator:
    ...
    def validate(self, ctx: SignalContext) -> ValidatorResult:
        ...
        # P(profitable | signal) ∝ P(bet) × P(long_confidence) × dominant_regime_prob
        ...
        if bayesian_score < 0.15:
            return ValidatorResult(
                ...
                f"(p_bet={ctx.p_bet:.3f} × dir_conf={direction_conf:.3f} × "
        ...
        if abs(implied_edge - ctx.expected_edge_bps) > 50:
            ...
            return ValidatorResult(
                self.NAME, ValidatorStatus.WARN,
...
class RiskValidator:
    ...
    def validate(self, ctx: SignalContext) -> ValidatorResult:
        ...
        # 3c. Volatility explosion gate (Schwager 1984 — 2× median ATR)
        vol_ratio = ctx.atr / ctx.atr_median_20 if ctx.atr_median_20 > 0 else 1.0
        metrics["atr_vol_ratio"] = round(vol_ratio, 4)
        if vol_ratio > 2.0:
            return ValidatorResult(
                self.NAME, ValidatorStatus.VETO,
                f"Volatility explosion: ATR ratio {vol_ratio:.2f} > 2.0× median — "
...
class BlockchainValidator:
    ...
    def validate(self, ctx: SignalContext) -> ValidatorResult:
        ...
        if abs(ctx.funding_rate_8h) > self.FUNDING_VETO_THRESHOLD:
            ...
            unfavorable = (
                (direction == "long"  and ctx.funding_rate_8h > 0) or
