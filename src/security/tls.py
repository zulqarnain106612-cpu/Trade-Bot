...
Turning certificate verification off is not a security decision, it is a
debugging step somebody forgot to undo. It appears when a staging certificate
expires and the fix is needed in ten minutes, and it survives because nothing
fails afterwards: the
...
post_quantum posture (LAW12):
  ...
  established with X25519 or elliptic-curve Diffie-Hellman key agreement
  today, and those are exactly
...
# Spelled as patterns rather than substrings so the spaced form and the
# keyword-argument form both match.  # arch-ignore
