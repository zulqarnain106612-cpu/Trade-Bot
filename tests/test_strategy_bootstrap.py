...
def test_default_fractions_do_not_over_commit_the_book():
...
def test_over_committed_fractions_raise_before_registering_anything():
    registry = StrategyRegistry()
    cfg = _cfg(
        signal_engine_enabled=True,
        signal_engine_fraction=1.0,
        breakout_enabled=True,
        breakout_fraction=0.5,
    )
    with pytest.raises(ValueError, match="more than the whole book"):
        register_default_strategies(registry, cfg)
    # Fail-fast means fail-clean: no partial portfolio left behind.
    assert len(registry) == 0


def test_fractions_summing_to_exactly_one_are_allowed():
    registry = StrategyRegistry()
    cfg = _cfg(
        signal_engine_enabled=True,
        signal_engine_fraction=0.6,
        breakout_enabled=True,
        breakout_fraction=0.4,
    )
